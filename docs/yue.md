...
**用途型可用性**：風控按用途而唔係關鍵詞——政治表達、公共人物、機構、國家、歷史題材同諷刺作品都可以出現；被攔嘅係認證用途同復刻現存官方印鑑嘅意圖。見 AUP.md。

## 使用流程

1. 寫好 `config.yaml`（schema 見 en.md）。AUP 風控檢驗喺渲染管線入面自動強制執行——冇得跳過，唔使單獨跑校驗器
2. 準備輸入相，放喺用例目錄入面，再喺 config 寫相對路徑（`photo: input.jpg`）
